...
| 5 | RB-033 | Long-term tracking (Journey adherence) | P0 | In Progress | S | TBD |
...
5. **RB-033 — Long-term tracking (Journey adherence)** — **In Progress.** Rename Weekly supports → **Long-term tracking**; add trackers w/ times/week; default/clear to **Medication only**; Journey adherence from trackers. Task-based Track over time → **Won't Do**. Effort **S**. Detail: [`items/long-term-tracking-adherence.md`](./items/long-term-tracking-adherence.md).
...
4. **RB-005 — Podcast-first + regular recovery content** — **Ready**; keep podcast useful + no-repeat off Home for now; founder still wants some regular recovery content (no duplicate item). Home On air **hidden** — slot is crossword (RB-024) / park RB-025. Detail: [`items/recovery-content-offers.md`](./items/recovery-content-offers.md).
...
**Won't Do:** RB-009 recovery patterns / craving analytics (founder: drop craving stats); **task-based Track over time** (superseded by RB-033 long-term trackers).  
...
| Now (personal tools + rituals) | RB-014 todos (**Ready / L**) → **RB-026 task groups + calendar M/D** (**In Progress / M**) → **RB-033 long-term tracking** (**In Progress / S**; supports→trackers, med-only default; task Track over time Won't Do) → **RB-032 Daily briefing Open/Close** (**Done / L**, shipped prod 2026-09-12; iterates Done RB-027/029/030) → RB-016 five-year journal → **RB-021 journal photos** | Merged Today’s Items + master page; groups/colors; Journey adherence via long-term trackers; ritual twin redesign shipped |
...
| Next (EA + Home) | RB-002 Gmail → **RB-024 daily puzzle** → RB-005 podcast (thin; **off Home**) → RB-017 cameras → **RB-028 Journey vitals** (**Done**) → **RB-031 med adherence + drop vitals chart** (**Ready / S**; fits RB-033 med-only default) → RB-018 workout → RB-019 recipes → RB-015 hub → RB-003 digest | Puzzle = **5×5** crossword replacing On Air; Start→attempt; banner `completed/attempts · success%` (**M**; not fill %); Entertainment park RB-025; **RB-023 calendar Done**; RB-031 = list-only vitals + SupportCompletion % days covered |
...
| Won't Do | RB-009; task Track over time (under RB-033) | Craving pattern analytics; longitudinal habits via tasks |
