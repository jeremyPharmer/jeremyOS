...
| 9 | RB-021 | Journal photos (attach + paperclip) | P1 | Backlog | S | TBD |
| 10 | RB-022 | Journal edit, star & month calendar | P0 | In Progress | M | TBD |
...
5. **RB-027 — Redesigned morning day-start** — **Done.** Locked 2026-09-10: five metrics all **1–10** (sleep hours unified); tap-a-number (no sliders); intention kept; post-check-in **templated briefing** (quote + day summary; weather / feeling / calendar / tasks / free time); suggest snoozed/deferred tasks into calendar gaps; **no** morning Items checklist (Home tasks). Evening counterpart is **RB-029**. Detail: [`items/morning-day-start-briefing.md`](./items/morning-day-start-briefing.md).
6. **RB-029 — Evening close success: day recap + world news** — **Done.** Intake 2026-09-11: empty-quotes Headline bug fixed; success screen is a **general day recap** + **3–5 major world news headlines**; journal `oneLine` stays personal. Rules-based v1. Detail: [`items/evening-close-recap-news.md`](./items/evening-close-recap-news.md).
7. **RB-030 — Conversational morning briefing (calendar-first)** — **Done.** Locked 2026-09-11 (ship-and-iterate on **RB-027 Done**): drop rigid labeled dashboard; **conversational** rules-based surface; **calendar + open windows / Try here** first; weather/feeling secondary; intention + 1–10 check-in unchanged; **no** morning Items checklist; **no** LLM. Detail: [`items/morning-briefing-conversational.md`](./items/morning-briefing-conversational.md).
8. **RB-016 — Five-year / paper journal UX** — same calendar day across years; headline + short summary; journal vibes (not stacked cards). Detail: [`items/five-year-journal-ux.md`](./items/five-year-journal-ux.md). Distinct from RB-010 backfill integrity.
9. **RB-021 — Journal photos (attach + paperclip)** — optional pics on journal/evening entries; paperclip (or similar) on year slots when a photo is present; tap to view; **reuse** existing photo infra. Detail: [`items/journal-photos.md`](./items/journal-photos.md).

**Mid-flight (finish thin; do not expand):** RB-011 (fund auto-credit), RB-010 (journal backfill) — personal-tool integrity, ranks 19–20. Do **not** widen RB-010 into edit-past ([RB-022](./items/journal-edit-star-calendar.md)).
...
9. LLM / generative morning briefing — after [RB-030](./items/morning-briefing-conversational.md) conversational rules-based iterate (parent [RB-027](./items/morning-day-start-briefing.md)).
10. LLM / generative evening recap — after [RB-029](./items/evening-close-recap-news.md) rules-based v1.

**Won't Do:** RB-009 recovery patterns / craving analytics (founder: drop craving stats).  
**Done:** RB-007 accounts + onboarding; **RB-020** drop craving stats + Home craving CTA; **RB-023** calendar iCal + Google (Home agenda + Settings; superseded env/Today’s Build PR #70 approach); **RB-027** morning day-start briefing; **RB-028** Journey vitals BP+HR; **RB-029** evening close recap + news; **RB-030** conversational morning briefing.
...
| Now (personal tools) | RB-014 todos (**Ready / L**) → **RB-026 task groups + calendar M/D** (**In Progress / M**, locked 2026-09-08; one first ship) → **RB-027 morning** (**Done**) → **RB-029 evening close recap + news** (**Done**) → **RB-030 conversational morning briefing** (**Done**) → RB-016 five-year journal → **RB-021 journal photos** | Merged Today’s Items + master page; groups/colors + Home no-completed; morning Done; evening = success-screen recap/news pair; journal cluster still in flight |
...
| Next (EA + Home) | RB-002 Gmail → **RB-024 daily puzzle** → RB-005 podcast (thin; **off Home**) → RB-017 cameras → **RB-028 Journey vitals** (**Done**) → RB-018 workout → RB-019 recipes → RB-015 hub → RB-003 digest | Puzzle = **5×5** crossword replacing On Air; Start→attempt; banner `completed/attempts · success%` (**M**; not fill %); Entertainment park RB-025; **RB-023 calendar Done** |
...
| Later / paused | RB-008, RB-004, **RB-025 Home Entertainment park**, RB-026 v2 group CRUD, RB-030/RB-029 LLM briefing/recap | Unrequested channels + Entertainment demoted; group add/rename after fixed v1 groups; generative copy after rules-based morning/evening |
...
| Done (rituals / vitals) | **RB-027** morning briefing; **RB-028** Journey vitals BP+HR; **RB-029** evening recap+news; **RB-030** conversational morning | Rituals shipped; LLM polish later |
