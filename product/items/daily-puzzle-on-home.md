...
- **2026-09-18 founder feedback:** same answers/clues (e.g. OPERA, ITEM) recycled too often. **Lock:** every answer word unique across the curated pack; regression test enforces pack-wide uniqueness. **Expand:** pack grown to 50+ puzzles (~two-month rotation) so the same words don’t return for extended periods; DATE_PUZZLES pins avoided when they would reuse pack answers mid-cycle.
- **2026-09-19 founder feedback:** grid felt stuck on 5-letter answers. **Lock:** majority ladder grids with 4+5 letter entries interleaved with classics; tests require ~45%+ ladders and substantial 4-letter share.
