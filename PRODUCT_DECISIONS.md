...
**Day** — Log **long-term trackers** (Settings/onboarding label; formerly Weekly supports): default / cleared set is **Medication only** (times/week target). Jeremy may add custom trackers with a frequency. Do **not** use tasks (“Track over time”) for longitudinal adherence — **RB-033**. **No** Home craving-timer CTA (RB-020). *(Runtime shipped: Settings/onboarding **Long-term tracking**, med-only default + one-time clear, Journey adherence from trackers; task Track over time removed from composer.)*  
