...
export function isAgendaEventPast(
  ...
  if (event.allDay || event.startTime === "All day" || event.startTime === "Anytime") {
    return false;
  }
...
export function orderAgendaUpcomingThenPast(
  ...
  const upcoming: WorkCalendarEvent[] = [];
  const past: WorkCalendarEvent[] = [];
